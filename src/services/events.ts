...
function mapRow(row: Record<string, unknown>): DbCityEvent {
  return {
    ...
    dateLabelAm: (row.date_label_am as string) || undefined,
    timeLabel: (row.time_label as string) || undefined,
    venue: row.venue as string,
    venueAm: (row.venue_am as string) || undefined,
    category: row.category as CityEvent['category'],
    description: row.description as string,
    descriptionAm: (row.description_am as string) || undefined,
    priceLabel: (row.price_label as string) || 'Check locally',
    priceLabelAm: (row.price_label_am as string) || undefined,
