...
> {
  ...
  try {
    const [places, reviews, claims, reports] = await Promise.all([
      supabase.from('places').select('id, name, status, updated_at').order('updated_at', { ascending: false }).limit(5),
      supabase.from('reviews').select('id, rating, status, created_at, place:places(name)').order('created_at', { ascending: false }).limit(4),
      supabase.from('place_claims').select('id, status, created_at, place:places(name)').order('created_at', { ascending: false }).limit(3),
      supabase.from('review_reports').select('id, reason, status, created_at').order('created_at', { ascending: false }).limit(3),
    ])
    for (const p of places.data ?? []) {
      items.push({ id: `place-${p.id}`, at: p.updated_at, action: `Place · ${p.status}`, detail: p.name })
    }
    for (const r of reviews.data ?? []) {
      const place = Array.isArray(r.place) ? r.place[0] : r.place
      items.push({ id: `review-${r.id}`, at: r.created_at, action: `Review · ${r.rating}★ · ${r.status}`, detail: (place as { name?: string } | null)?.name ?? 'Review' })
    }
    for (const c of claims.data ?? []) {
      const place = Array.isArray(c.place) ? c.place[0] : c.place
      items.push({ id: `claim-${c.id}`, at: c.created_at, action: `Claim · ${c.status}`, detail: (place as { name?: string } | null)?.name ?? 'Claim' })
    }
    for (const r of reports.data ?? []) {
      items.push({ id: `report-${r.id}`, at: r.created_at, action: `Report · ${r.status}`, detail: r.reason ?? 'Report' })
    }
...
export async function fetchOpenReports(limit = 80) {
  const { data, error } = await supabase
    .from('review_reports')
    .select('*, review:reviews(id, comment, rating, status, place:places(name, slug)), reporter:profiles(full_name, email)')
    .order('created_at', { ascending: false })
    .limit(limit)
  if (error) {
...
export const PROFILE_ROLES = ['visitor', 'contributor', 'business', 'moderator', 'admin'] as const
