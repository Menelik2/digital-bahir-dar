...
export function PlaceBottomSheet({ place, distanceM, onClose, onDirections, className }: Props) {
  ...
  return (
    <div
      className={cn(
        // z above map filters so sheet is never hidden / "blank" under controls
        'absolute bottom-20 left-0 right-0 z-[1100] rounded-t-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900',
        'pb-safe lg:bottom-6 lg:left-4 lg:right-auto lg:w-96 lg:rounded-2xl',
        className
      )}
    >
      <div className="flex items-start justify-between p-4 pb-2">
        <div className="min-w-0 flex-1">
          ...
          <h3 className="truncate text-lg font-semibold text-slate-900 dark:text-white">{name}</h3>
        ...
        <button
          type="button"
          onClick={onClose}
          className="ml-2 rounded-lg p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800"
          aria-label="Close"
        >
