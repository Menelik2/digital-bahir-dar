...
export default function PlaceDetailsPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-slate-500">
        <Loader2 className="mb-3 h-8 w-8 animate-spin text-[#078930]" /> Loading place…
      </div>
    )
  }
  ...
  return (
    <div className="min-h-full bg-[#f2f2f7] pb-nav-safe dark:bg-black">
      <div className="relative h-56 overflow-hidden bg-slate-800 sm:h-72 lg:h-80">
        ...
        <div className="absolute left-4 top-4 z-10">
          <button type="button" onClick={() => navigate(-1)}>
            <Button size="icon" variant="outline" className="h-11 w-11 rounded-full border-0 bg-white/90 shadow-md backdrop-blur active:scale-95">
      ...
      <div className="mx-auto max-w-3xl px-4 py-5 sm:py-6">
        <div className="mb-5 flex flex-col gap-3 sm:mb-6 sm:flex-row sm:flex-wrap sm:items-center">
          {distanceM != null && (
            <span className="flex items-center gap-1.5 text-[13px] font-medium text-[#3c3c43] dark:text-white/80">
          ...
          <section className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {info.highlights.split(' · ').map((h) => (
                <span key={h} className="rounded-full bg-emerald-50 px-3 py-1.5 text-sm font-medium text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-200">
                  {h.trim()}
                </span>
              ))}
            </div>
          ...
          <section className="mb-8">
            ...
            <ul className="space-y-2">
              {info.tips.split(' · ').map((tip) => (
                <li key={tip} className="flex gap-2 rounded-xl bg-white px-3.5 py-2.5 text-sm text-slate-700 shadow-sm dark:bg-[#1c1c1e] dark:text-slate-200">
          ...
          <section className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {info.bring.split(',').map((item) => (
                <span key={item} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-sm dark:border-slate-700 dark:bg-[#1c1c1e]">
                  {item.trim()}
                </span>
              ))}
            </div>
          ...
          <section className="mb-8">
            ...
            <Card>
              <CardContent className="grid gap-3 p-4 sm:grid-cols-2">
                ...
                {place.hotel.amenities && place.hotel.amenities.length > 0 && (
