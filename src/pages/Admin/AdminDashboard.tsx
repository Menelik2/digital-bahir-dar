  ...
  if (!isAuthenticated || !isStaff) {
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        ...
        <div className="mt-6">
          <Link to="/auth" className="inline-flex items-center justify-center rounded-md bg-sky-600 px-4 py-2 text-sm font-medium text-white hover:bg-sky-700">
            Sign in
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 lg:flex-row">
        ...
        <main className="min-w-0 flex-1 space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                <Button
                  ...
                  onClick={() => {
                    const rows = [
                      ['id', 'name', 'status', 'verified', 'featured'],
                      ...places.map((p) => [p.id, p.name, p.status, p.verified ? 'yes' : 'no', p.featured ? 'yes' : 'no']),
                    ]
                ...
                <Card key={p.id}>
                  <CardContent className="flex flex-col gap-2 p-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-1">
                      ...
                      <Button size="sm" variant="outline" onClick={async () => { await actions.placeUpdate.mutateAsync({ placeId: p.id, data: { name: p.name, featured: !p.featured } }); flash(p.featured ? 'Unfeatured' : 'Featured') }}>
                ...
                <Card key={r.id}>
                  <CardContent className="p-3">
                    ...
                    <div className="mt-2 flex gap-2">
                      {r.status !== 'published' && (
                        <Button size="sm" onClick={async () => { await actions.reviewStatus.mutateAsync({ reviewId: r.id, status: 'published' }); flash('Published') }}>Publish</Button>
                      )}
                      {r.status !== 'hidden' && (
          ...
          {tab === 'transport' && (
            <p className="text-sm text-slate-500">Transport list loads when expanded in a future update.</p>
          )}
        </main>
      </div>
    </div>
