...
const MODES: {
...
}[] = [
  {
    ...
    tileOn: 'bg-orange-500 border-orange-500 text-white shadow-md shadow-orange-200/60 dark:shadow-none',
  ...
  },
  {
    ...
    tileOn: 'bg-amber-600 border-amber-600 text-white shadow-md shadow-amber-200/60 dark:shadow-none',
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-2 pt-4">
      ...
      <div className="mb-4">
        ...
        <p className="mt-0.5 text-sm text-slate-500">Tap one · see matches below</p>
      </div>

      {/* Easy 3-up mood tiles — equal width, short height */}
        ...
        {MODES.map((m) => {
          ...
          return (
            ...
            >
              ...
              >
                <Icon
                  className={cn('h-4.5 w-4.5', on ? 'text-white' : m.iconColor)}
                  style={{ width: 18, height: 18 }}
                  strokeWidth={2.25}
                />
              </span>
              <span className={cn('text-[12px] font-bold leading-tight', on ? 'text-white' : 'text-slate-800 dark:text-slate-100')}>
                {m.title}
              </span>
              <span className={cn('text-[10px] leading-tight', on ? 'text-white/85' : 'text-slate-500')}>
                {m.hint}
              </span>
            </button>
          )
      ...
      {rest.length > 0 && mode === 'all' && (
        <>
          <h3 className="mb-2 text-[15px] font-semibold">More ideas</h3>
            ...
            {rest.map((r) => (
              <Card key={r.id}>
                <CardContent className="p-3.5">
                  ...
                  {r.slug && (
                    <Link to={`/places/${r.slug}`} className="mt-1.5 inline-block text-[11px] font-medium text-sky-600">
                      View →
                    </Link>
                  )}
                </CardContent>
      ...
      <h3 className="mb-2 text-[15px] font-semibold">Neighborhoods</h3>
        ...
        <div className="mb-6 grid gap-3 lg:grid-cols-2">
          <Card>
            <CardContent className="p-4">
              <h4 className="text-sm font-semibold">Etiquette</h4>
      ...
      <h3 className="mb-1 text-[15px] font-semibold">Live listings</h3>
      <p className="mb-2 text-xs text-slate-500">App + OpenStreetMap places nearby</p>
    </div>
  )
}
