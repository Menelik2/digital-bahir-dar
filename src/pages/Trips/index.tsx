...
/** Amharic titles for ready-made guide cards (UI only) */
const GUIDE_AM: Record<string, { title: string; subtitle: string; highlights: string[] }> = {
  'guide-perfect-day': {
    title: 'ፍጹም 1 ቀን በባሕር ዳር',
    subtitle: 'የሐይቅ ዳር፣ እይታ፣ ገበያ እና ትክክለኛ የኢትዮጵያ እራት',
    highlights: ['የጣና ዳር ጉዞ', 'በዛዊት እይታ', 'ማዕከላዊ ገበያ', 'ዓሳ ወይም እንጀራ እራት'],
  },
  'guide-weekend-classic': {
    title: 'ክላሲክ ሳምንት መጨረሻ (2 ቀን)',
    subtitle: 'ቀን 1 ሐይቅ እና ገዳማት · ቀን 2 የአባይ ፏፏቴ',
    highlights: ['የጣና ገዳም ጀልባ', 'ጢስሳት (የአባይ ፏፏቴ)', 'የሐይቅ ማታዎች'],
  },
  'guide-3-day-complete': {
    title: 'ሙሉ ባሕር ዳር (3 ቀን)',
    subtitle: 'ከተማ + ገዳማት + ፏፏቴ — ሚዛናዊ ጉዞ',
    highlights: ['ሙሉ የሐይቅ ቀን', 'የፏፏቴ ቀን', 'ነጻ የከተማ ጠዋት', 'ገበያ እና ምግብ'],
  },
  'guide-budget-backpacker': {
    title: 'በጀት አሳሽ (2–3 ቀን)',
    subtitle: 'ባጃጅ፣ የአካባቢ ምግብ፣ የጋራ ጀልባ — ባሕር ዳር በዝቅተኛ ወጪ',
    highlights: ['ጎስት ሃውስ', 'እንጀራ ምግብ', 'የጋራ የፏፏቴ ትራንስፖርት', 'በእግር ጉብኝት'],
  },
  'guide-slow-culture': {
    title: 'ዝግ ባህል (4 ቀን)',
    subtitle: 'ተጨማሪ የሐይቅ ጊዜ፣ እደ-ጥበብ፣ ምግብ እና እረፍት',
    highlights: ['ሁለት ቀላል ጀልባዎች', 'ገበያ እና ቡና', 'አማራጭ ሁለተኛ ገዳም', 'ያለ ጠዋት ማንቂያ ፏፏቴ'],
  },
  'guide-nature-focus': {
    title: 'ተፈጥሮ ትኩረት (2 ቀን)',
    subtitle: 'ወፎች፣ የሐይቅ ዳር፣ ፏፏቴ — ያነሰ ግዢ፣ ተጨማሪ ከቤት ውጭ',
    highlights: ['የንጋት የሐይቅ ብርሃን', 'የፏፏቴ ርጭት ዞን', 'በዳር ያሉ ወፎች'],
  },
}
...
function GuideCard({ guide }: { guide: GuideItinerary }) {
  ...
  const am = language === 'am' ? GUIDE_AM[guide.id] : undefined
  const title = am?.title ?? guide.title
  const subtitle = am?.subtitle ?? guide.subtitle
  const highlights = am?.highlights ?? guide.highlights
  const paceLabel =
    guide.pace === 'relaxed'
      ? t.planner.paceRelaxed
      : guide.pace === 'active'
        ? t.planner.paceActive
        : t.planner.paceModerate
      ...
      <Card className="h-full overflow-hidden border-[#078930]/15 transition hover:border-[#078930]/40 hover:shadow-md dark:border-[#078930]/30">
        ...
        <CardContent className="flex h-full flex-col p-4">
          <div className="mb-2 flex items-start justify-between gap-2">
            <h3 className="font-semibold leading-snug text-slate-900 dark:text-white">{title}</h3>
            <span className="shrink-0 rounded-full bg-[#078930]/10 px-2 py-0.5 text-[10px] font-semibold text-[#056b24] dark:text-[#7dcea0]">
              {guide.days}{t.trips.daysShort}
            </span>
          </div>
          <p className="mb-3 line-clamp-2 text-sm text-slate-500">{subtitle}</p>
          ...
          <div className="mt-auto flex flex-wrap items-center gap-3 text-[11px] text-slate-500">
            <span className="flex items-center gap-1">
              <Clock className="h-3 w-3" /> {paceLabel}
            </span>
            <span className="flex items-center gap-1">
              <Wallet className="h-3 w-3" />~
              {guide.budgetPerPersonEtb.typical.toLocaleString()} {t.trips.perPersonShort}
            </span>
          </div>
        </CardContent>
      </Card>
...
export default function TripsPage() {
  ...
  const tagOptions = [
    ...
    { id: 'budget', label: t.planner.budgetBudget },
