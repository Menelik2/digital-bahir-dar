    ...
    <div className="bg-[#f2f2f7] dark:bg-black">
      <section className="relative min-h-[320px] overflow-hidden text-white sm:min-h-[380px] lg:min-h-[440px]">
        <div className="absolute inset-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-br from-black/55 via-[#0a4d6e]/35 to-[#056b24]/30" aria-hidden />
          <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/20 to-black/15" aria-hidden />
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_transparent_0%,_rgba(0,0,0,0.25)_70%)]" aria-hidden />
        </div>
        ...
        <div className="relative z-10 mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 sm:py-16 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:gap-12 lg:py-24 xl:px-8">
          <div className="text-center lg:text-left">
            ...
            <div className="flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:items-center lg:justify-start">
              <Link to="/today" className="w-full sm:w-auto">
                <Button size="lg" className="h-12 w-full rounded-full bg-[#f5c518] px-7 text-[#3d3200] shadow-lg shadow-black/25 transition active:scale-[0.98] hover:bg-[#e6b800] sm:w-auto">
                  <Sun className="h-5 w-5" /> {t.home.todayTitle}
                </Button>
              </Link>
              <Link to="/trip-planner" className="w-full sm:w-auto">
                <Button size="lg" variant="outline" className="h-12 w-full rounded-full border-white/40 bg-white/15 px-7 text-white backdrop-blur-md transition active:scale-[0.98] hover:bg-white/25 sm:w-auto">
          ...
          <div className="hidden lg:grid lg:grid-cols-2 lg:gap-3">
            {desk.map((item) => (
              <Link key={item.to} to={item.to} className="group relative overflow-hidden rounded-[1.25rem] p-4 shadow-lg ring-1 ring-white/25 transition duration-200 hover:-translate-y-1 hover:shadow-xl hover:ring-white/40 active:translate-y-0 active:scale-[0.98]">
                <img src={item.image} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={(e) => { const el = e.currentTarget; if (el.dataset.fb !== '1') { el.dataset.fb = '1'; el.src = item.imageFb } }} />
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-10 lg:py-14 xl:px-8">
        <div className="mb-4 sm:mb-6 lg:mb-8">
          <p className="mb-1 hidden text-[12px] font-semibold uppercase tracking-wide text-[#078930] dark:text-[#30d158] lg:block">
            {t.home.startHere}
          </p>
          <h2 className="text-[20px] font-bold tracking-tight text-[#1c1c1e] dark:text-white sm:text-2xl lg:text-[28px]">
            {t.home.whatNeed}
          </h2>
          <p className="mt-1 max-w-2xl text-[13px] leading-relaxed text-[#8e8e93] sm:text-sm lg:text-[15px]">
            {t.home.whatNeedSub}
          </p>
          <p className="mt-2 hidden text-[13px] text-[#3c3c43]/80 dark:text-white/55 lg:block">
            {t.home.startHereSub}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:hidden">
          {desk.map((item) => (
            <Link key={item.to} to={item.to} className="ios-press group block [perspective:900px]">
              <div className={cn('relative flex min-h-[140px] flex-col justify-end overflow-hidden rounded-[1.4rem] text-white sm:min-h-[160px]', 'shadow-[0_10px_28px_-8px_rgba(0,0,0,0.45),0_2px_6px_rgba(0,0,0,0.2)]', 'ring-1 ring-black/10 transition duration-300', 'group-hover:[transform:rotateX(2deg)_translateY(-2px)] group-active:scale-[0.97]', item.ring)}>
                <img src={item.image} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-110" loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={(e) => { const el = e.currentTarget; if (el.dataset.fb !== '1') { el.dataset.fb = '1'; el.src = item.imageFb } }} />
        ...
        <div className="hidden lg:grid lg:grid-cols-4 lg:gap-4 xl:gap-5">
          {desk.map((item) => (
            <Link key={item.to} to={item.to} className="ios-press group block [perspective:1200px]">
              <div className={cn('relative flex h-full min-h-[220px] flex-col overflow-hidden rounded-[1.6rem] text-white', 'shadow-[0_14px_40px_-12px_rgba(0,0,0,0.4),0_4px_12px_rgba(0,0,0,0.15)]', 'ring-1 ring-black/10 transition duration-300 ease-out', 'group-hover:-translate-y-1.5 group-hover:shadow-[0_22px_50px_-14px_rgba(0,0,0,0.5)]', 'group-hover:[transform:translateY(-6px)_rotateX(3deg)]', item.ring)}>
                <img src={item.image} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-110" loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={(e) => { const el = e.currentTarget; if (el.dataset.fb !== '1') { el.dataset.fb = '1'; el.src = item.imageFb } }} />
                <div className={cn('absolute inset-0 bg-gradient-to-t', item.gradient)} aria-hidden />
      ...
      <section className="border-y border-black/[0.03] bg-white/60 px-4 py-6 dark:border-white/[0.06] dark:bg-[#0c0c0e]/80 sm:px-6 sm:py-10 xl:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-4 sm:mb-6">
            <h2 className="text-[20px] font-bold tracking-tight text-[#1c1c1e] dark:text-white sm:text-2xl">{t.home.moreTools}</h2>
            <p className="mt-1 hidden text-[14px] text-[#8e8e93] lg:block">{t.nav.more}</p>
          </div>
