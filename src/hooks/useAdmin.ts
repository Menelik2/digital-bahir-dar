...
import type { TripInput, TripStatus } from '@/types/trip'
...
export function useAdminTrips(enabled: boolean) {
  return useQuery({
    queryKey: ['admin-trips'],
    queryFn: () => fetchAdminTrips(150),
    enabled,
    staleTime: 20_000,
  })
}
...
export function useAdminTripDetail(tripId: string | null) {
  return useQuery({
    ...
    queryFn: () => (tripId ? fetchTrip(tripId) : null),
    enabled: !!tripId,
    staleTime: 10_000,
  })
}
...
export function useAdminActions() {
  ...
  const tripCreate = useMutation({
    mutationFn: async (args: { userId: string; data: TripInput & { is_public?: boolean } }) => {
      const res = await adminCreateTrip(args.userId, args.data)
      if (res.error) throw new Error(res.error)
      return res.trip
    },
    onSuccess: invalidate,
  ...
  const tripUpdate = useMutation({
    mutationFn: async (args: { tripId: string; data: Partial<TripInput> & { is_public?: boolean } }) => {
      const res = await adminUpdateTrip(args.tripId, args.data)
      if (res.error) throw new Error(res.error)
    },
    onSuccess: invalidate,
  })
  ...
  const tripStatus = useMutation({
    mutationFn: async (args: { tripId: string; status: TripStatus }) => {
      const res = await adminSetTripStatus(args.tripId, args.status)
      if (res.error) throw new Error(res.error)
    },
    onSuccess: invalidate,
  })
  ...
  const tripAddDay = useMutation({
    mutationFn: async (args: { tripId: string; dayNumber: number; title?: string }) => {
      const res = await addTripDay(args.tripId, args.dayNumber, { title: args.title })
      if (res.error) throw new Error(res.error)
      return res.day
    },
    onSuccess: invalidate,
  ...
  const tripAddStop = useMutation({
    mutationFn: async (args: { tripDayId: string; custom_name: string; notes?: string; estimated_cost?: number }) => {
      const res = await addTripStop(args.tripDayId, {
  ...
  const tripDeleteStop = useMutation({
  ...
  const tripDeleteDay = useMutation({
    mutationFn: async (dayId: string) => {
      const res = await adminDeleteTripDay(dayId)
      if (res.error) throw new Error(res.error)
    },
    onSuccess: invalidate,
  })
